#!/usr/bin/env python3
"""Recompute signal-state diagnostic fields for an existing phasic run."""
...
from photometry_pipeline.core.signal_state_diagnostics import (  # noqa: E402
    DEFAULTS as SIGNAL_STATE_DEFAULTS,
...
SIGNAL_STATE_FIELD_NAMES = set(
    compute_signal_state_diagnostics(np.arange(10.0, dtype=float)).keys()
)
SIGNAL_STATE_LIST_FIELDS = {"signal_state_flags"}
FLAG_LIST_FIELDS = {
...
def _load_signal_state_config(phasic_path: Path) -> tuple[dict[str, Any], bool, str | None]:
    for path in _candidate_config_paths(phasic_path):
        ...
        config = {
            key: data[key]
            for key in SIGNAL_STATE_DEFAULTS
            if key in data
        }
...
def _merge_signal_state_fields(record: dict[str, Any], diagnostics: dict[str, Any]) -> dict[str, Any]:
    out = dict(record)
    for key in SIGNAL_STATE_FIELD_NAMES:
        out.pop(key, None)
...
def _records_to_csv(records: list[dict[str, Any]], columns: list[str]) -> str:
    ...
    for field in sorted(SIGNAL_STATE_FIELD_NAMES):
...
def _records_to_json(records: list[dict[str, Any]]) -> str:
    ...
    for rec in records:
        out = dict(rec)
        for key, value in list(out.items()):
            if key in SIGNAL_STATE_LIST_FIELDS or key.startswith("proposal_flags_"):
...
def recompute_signal_state_diagnostics(
    ...
    *,
    ...
    config, using_defaults, config_source = _load_signal_state_config(phasic_path)
    ...
    with open_phasic_cache(str(cache_path)) as cache:
        for raw_rec in records:
            ...
            try:
                ...
                continue
            diagnostics = compute_signal_state_diagnostics(
                signal=np.asarray(sig_raw, dtype=float),
                time=np.asarray(time_sec, dtype=float),
                config=config,
            )
            updated.append(_merge_signal_state_fields(rec, diagnostics))
    if missing:
        raise RuntimeError(
            "Cannot recompute signal-state diagnostics because raw signal/time data are "
    ...
    signal_summary = summarize_signal_state_diagnostics(updated)
    signal_summary["using_default_signal_state_config"] = bool(using_defaults)
    signal_summary["signal_state_config_source"] = config_source
    qc_summary["signal_state_diagnostics_summary"] = signal_summary
    ...
    return {
        ...
        "using_default_signal_state_config": bool(using_defaults),
        "signal_state_config_source": config_source,
        "warnings": warnings,
...
def _print_report(report: dict[str, Any]) -> None:
    ...
    print(
        "using_default_signal_state_config: "
        f"{str(report['using_default_signal_state_config']).lower()}"
    )
    if report.get("signal_state_config_source"):
        print(f"signal_state_config_source: {report['signal_state_config_source']}")
    for warning in report.get("warnings", []):
        print(f"WARNING: {warning}")
    summary = report.get("summary", {})
    print(f"signal_state_candidate_class_counts: {summary.get('signal_state_candidate_class_counts', {})}")
    print(f"signal_state_flag_counts: {summary.get('signal_state_flag_counts', {})}")


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Recompute signal-state diagnostic fields from an existing "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = recompute_signal_state_diagnostics(
            args.phasic_out,
